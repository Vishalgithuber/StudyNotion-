...
exports.changePassword = async (req, res) => {
    try {
        ...
        // Get old and new password , and confirm new password from req.body
        const { oldPassword, newPassword } = req.body;

        // validate the oldpasswrod
        const isPasswordMatch = await bcrypt.compare(
            oldPassword,
            userDetails.password
        );
        if (!isPasswordMatch) {
            // If old password does not match , return
            return res.status(401).json({
                success: false,
        ...
        //update the password
        const encryptrdPassword = await bcrypt.hash(newPassword, 10);
        ...
        // send notificatoin email
        try {
            emailResponsee = await mailSender(
